...
documents = []
texts = []
dictionary = corpora.Dictionary.load('tmp/cand_deerwester.dict')
corpus = corpora.MmCorpus('tmp/cand_deerwester.mm')
...
class Method:
    ...
    def choose_method(self):
        if self.method == 'lda':
            ...
            index.save('tmp/cand_deerwester.index')
            index = similarities.MatrixSimilarity.load('tmp/cand_deerwester.index')
            sims = index[vec_lda]
